...
const { computeLayout, computeTextOnlyLayout } = require('./layoutEngine');
...
/**
 * Compiler for typography-only designs — apparel quote tees with no
 * central graphic. Resolves niche style, computes a text-only layout
 * via computeTextOnlyLayout(), and builds just the text layer (no
 * asset layer at all), wrapped in the same SVG/optimize pipeline as
 * compileDesignSvg().
 */
function compileTypographyDesignSvg({
  text,
  niche,
  paletteOverride,
  transparentBackground = true,
  ...
  const style = resolveStyle({ niche, paletteOverride });
  const layout = computeTextOnlyLayout({ text });
...
module.exports = { compileDesignSvg, compileTypographyDesignSvg };
