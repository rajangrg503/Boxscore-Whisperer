...
The visible multiselect widget itself is untouched -- only the
underlying calculation and rendering loops are pointed at a merged
list instead of the raw widget value.

Corrected version: fixes the anchor indentation (12 spaces, not 4)
that caused the first attempt to find 0 matches.

Usage:
    python3 patch_defender_autolink_v2.py
